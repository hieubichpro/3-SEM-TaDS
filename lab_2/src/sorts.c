...
void bubble_sort_flats(flat_t *flats_arr, int size)
{
    for (int i = 0; i < size - 1; i++)
    {
        for (int j = 0; j < size - i - 1; j++)
...
{
    return ((flat_t *)(val1))->square - ((flat_t *)(val2))->square;
}
...
void qsort_table(table_t table)
{
    ...
    qsort(table.flats_arr, table.size, sizeof(flat_t), comparator_table);
...
short int compare_sorts(FILE *flats_file)
{
    ...
    start3 = tick_count();
    qsort(flats_arr, size, sizeof(flat_t), comparator_table);
    end3 = tick_count();
    ...
    printf("при сортировки таблицы:  qsort быстрее чем bubble sort %lf%c \n", (double)(((end1 - start1) - (end3 - start3)) * 100) / (end1 - start1), 37);
    printf("при сортировки таблицы ключей:  qsort быстрее чем bubble sort %lf%c\n", (double)(((end2 - start2) - (end4 - start4)) * 100) / (end2 - start2), 37);
